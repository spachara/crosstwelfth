	...
	<section class="container_content">
    	...
    	<section class="paragraph">
            <hgroup class="titleModule">
                <h2>ผลการค้นหา
					<?php
					 $sql_product = "SELECT * FROM product_tb where ranking not in ('','0')";
					
					if($_GET['chkCat'] != '' ){
                     $sql_product .= " and p_category in (".$categorArr.") ";
					}
					...
					if($_GET['chkCode'] != '' ){
                     $sql_product .= " and p_code like '%".$_GET['chkCode']."%'";
					}
					if($_GET['chkName'] != '' ){
                     $sql_product .= " and name like '%".$_GET['chkName']."%'";
					}
					 
					 
					
					if($_GET['chkStatus'] == 'PRE' ){
                     $sql_product .= " and p_stock = '0' and (p_pre <> '0' or p_spare <> '0') ";
					}
					elseif($_GET['chkStatus'] == 'IN' ){
                     $sql_product .= " and p_stock > '0' ";
					}
                                        else{
                      $sql_product .= " and ( p_stock > '0' or p_pre <> '0' or p_spare <> '0' )";                       
                                            
                                        }                                         


					$sql_product .= " GROUP BY p_color, p_code  ORDER BY pid desc";
					//echo $sql_product;
                    $result_product =@mysql_query($sql_product, $connect);
                    $num_product =@mysql_num_rows($result_product);
				 
                    $result_product3 =@mysql_query($sql_product, $connect);
                    $num_product3 =@mysql_num_rows($result_product3);
                        $array_product_id[] = '';
					for($i=1;$i<=intval($num_product3);$i++){
                    	$data_product3 =@mysql_fetch_array($result_product3);
						
						
						
						$sql_color2 = "SELECT p.* FROM product_tb as p, size_tb as s, category_tb as c where c.name = p.p_category and c.cid = s.cid AND ";
						$sql_color2 .= " p.p_code = '".$data_product3['p_code']."'";
						$sql_color2 .= " AND p.p_color = '".$data_product3['p_color']."' AND p.p_size = s.name GROUP BY p.p_size ORDER BY s.ranking";
						
                        $result_color2 = @mysql_query($sql_color2, $connect);
                        $num_color2 =@mysql_num_rows($result_color2);
						
						$result_size52 = @mysql_query($sql_color2,$connect);
								
								for($g=1;$g<=intval($num_color2);$g++){
								$data_g2 =@mysql_fetch_array($result_size52);
						...
						for($col=1;$col<=intval($num_color2);$col++){
                            $data_color2 =@mysql_fetch_array($result_color2);
							
							if($data_color2['p_stock'] != '0'){
								  //echo "Ready to ship สินค้าพร้อมส่ง";
							  }elseif($finishSpare2 == 'OUT' && ($data_color2['p_stock'] == '0' && $data_color2['p_pre'] == '0')){
								  //echo  "Out of stock สินค้าหมดสต๊อก";
								  $out++;
							  }elseif($data_color2['p_stock'] == '0' && $data_color2['p_pre'] != '0'){
								  //echo  "Preorder จัดส่ง 10-15 วัน หลังชำระเงิน";
							  }elseif($data_color2['p_pre'] == '0' && $data_color2['p_spare'] != '0'){
								 // echo  "Preorder จัดส่ง 10-15 วัน หลังชำระเงิน";
							  }elseif($data_color2['p_stock'] == '0' && $data_color2['p_pre'] == '0'  && $data_color2['p_spare'] == '0' ){
								  //echo  "Out of stock สินค้าหมดสต๊อก";
								  $out2++;
							  }
						}
						if($num_color2==$out2){
							//echo "OUT OF STOCK";
							$chk_out_of += 1;
							$array_product_id[] = $data_product3['pid'];
						} else {
							//echo "IN STOCK";
						}
					}
                ...
                echo ($_GET['chkCode'] != '' ? " Code : ".$_GET['chkCode'] : "");
                echo ($_GET['chkName'] != '' ? " Name : ".$_GET['chkName'] : "");
                echo ($_GET['chkStatus'] == 'PRE' ? " งานสั่งตัด รอสินค้า 10-15 วัน " : "");
                echo ($_GET['chkStatus'] == 'IN' ? " สินค้ามีสต๊อก พร้อมส่ง " : "");
				echo "&nbsp;พบ ".($num_product-$chk_out_of)." แบบ";	
				?>
                </h2>
            </hgroup>        
            <section class="paragraph03 thumbPD" id="newarrival">
				...
				<?php                    
					...
					if (in_array($data_product['pid'], $array_product_id) == false) {
                    ?>
                    <li>
