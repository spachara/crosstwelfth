<?php
	...
	$sql_qa = "SELECT * FROM txt_tb WHERE type_name = 'qa'";
	$result_qa =@mysql_query($sql_qa, $connect);
	$data_qa =@mysql_fetch_array($result_qa);
	...
	<section class="container_content">
    	...
    	<section class="paragraph overflowNone">
            <hgroup class="titleModule">
                <h2>
				<?php 
                echo ($_SESSION['sess_language'] == 'eng' ? "Q & A" : "คำถามที่พบบ่อย");
                ?>
                </h2>
            </hgroup>        
            <section class="paragraph03">
				<?php 
                echo ($_SESSION['sess_language'] == 'eng' ? $data_qa['txt_detail_eng'] : $data_qa['txt_detail_th']);
                ?>
            </section>
            <div class="clear"></div>
        </section>
