	...
	$aa = explode(':',$i);
		...
		if($i != ''){//ถ้าหากไม่พบ $i ก็ไม่ต้องแสดงข้อมูล ($i คือ ไอดีที่กำหนดไว้ตอน Foreach >>> foreach($_SESSION['session_id'] as $key=> $i) )
			...
			$order_id += 1;
				...
				$insert_order_product .= ", '".mysql_real_escape_string($_SESSION['cus_phone'])."', NOW(), NOW())";
...
if($_SESSION['session_id']){
			$link = 'http://www.crosstwelfth.com/opf/index.php?o=' . $_SESSION['order_id'] . '&p=' . $_SESSION['cus_phone'] ;
			...
			$insert_order .= " , order_type_pay, order_employee, order_email, order_phone, order_group, link_url , date_in, date_update,order_shipping)";
			...
			$insert_order .= "  , '".$_SESSION['sess_paid']."', '".mysql_real_escape_string($_SESSION['cus_phone'])."' ";
			$insert_order .= " , '', '".mysql_real_escape_string($_SESSION['cus_phone'])."', '".$pro_group."', '" . $link . "' , NOW(), NOW() , '".mysql_real_escape_string($_SESSION['session_shipping'])."' )"; 
	...
	$aa = explode(':',$i);
		...
		if($i != ''){//ถ้าหากไม่พบ $i ก็ไม่ต้องแสดงข้อมูล ($i คือ ไอดีที่กำหนดไว้ตอน Foreach >>> foreach($_SESSION['session_pre_id'] as $key=> $i) )
			...
			$order_id += 1;
				...
				$insert_order_product2 .= ", '".mysql_real_escape_string($_SESSION['cus_phone'])."', NOW(), NOW())";
...
$cost_ship = $_SESSION['session_realShipPre'];
			...
			$link = 'http://www.crosstwelfth.com/opf/index.php?o=' . $_SESSION['order_pre_id'] . '&p=' . $_SESSION['cus_phone'] ;
			...
			$insert_order2 .= " , order_type_pay, order_employee, order_email, order_phone, order_group, link_url , date_in, date_update,order_shipping)";
			...
			$insert_order2 .= " , '".$cost_ship."', '".mysql_real_escape_string($_SESSION['cus_province'])."', '".$_SESSION['cus_country']."', '".$_SESSION['sess_paid']."', '".mysql_real_escape_string($_SESSION['cus_phone'])."' ";
			$insert_order2 .= " , '', '".mysql_real_escape_string($_SESSION['cus_phone'])."', '".$pre_group."' , '" . $link . "' , NOW(), NOW() , '".mysql_real_escape_string($_SESSION['session_shipping'])."' )"; 
			@mysql_query($insert_order2, $connect);

			if(mysql_real_escape_string($_SESSION['session_shipping']) == '3'){
				$update_order = "UPDATE order_tb set order_status = '3' ,tranfer_status = '6' ,payment_status = '6' Where order_shipping = '3' ";
				@mysql_query($update_order, $connect);
				}
